...
public class Formater {
	private static final Logger logger = Logger.getLogger(Formater.class);
	// The likely cause is the fact that SimpleDateFormat isn't threadsafe
	...
	private static final SimpleDateFormat sdf4Timestamp = new SimpleDateFormat("yyyyMMddHHmm");
	...
	public static Date str2date(String str) throws Exception {
		...
		try {
			...
			logger.error(String.format("Error parse date %s", str), e);
			throw e;
		}
	}
	...
	public static String date2str(Date d) {
		if (d == null)
			return "";
		return new SimpleDateFormat(sdf).format(d);
	}
	...
	public static String date2TimeStamp(Date d) {
		return sdf4Timestamp.format(d);
	}
	...
	public static Timestamp str2time(String str) throws Exception {
		...
		try {
			result = new Timestamp(new SimpleDateFormat(sdf).parse(str).getTime());
		} catch (Exception e) {
			throw new Exception(e.toString());
		}
		return result;
	...
	public static String time2str(Timestamp t) {
		if (t == null)
			return null;
		return new SimpleDateFormat(sdf).format(t);
	}

	public static String gc2str(GregorianCalendar gc) {
		if (gc == null)
			return null;
		return new SimpleDateFormat(sdf).format(gc.getTime());
	}
	...
	public static String num2str(BigDecimal number) {
		return FormatNumber.num2Str(number);
	}
	...
	public static void addDate(Date root, int soNgay) {
		System.out.println("Date is : " + new SimpleDateFormat(sdf).format(root));
		GregorianCalendar c1 = new GregorianCalendar();
		c1.setTime(root);
		c1.add(Calendar.DAY_OF_MONTH, soNgay);
		System.out.println("Date after " + soNgay + " is : " + new SimpleDateFormat(sdf).format(c1.getTime()));
	}

	public static void subDate(Date root, int soNgay) {
		System.out.println("Now is : " + new SimpleDateFormat(sdf).format(root));
		GregorianCalendar c1 = new GregorianCalendar();
		c1.setTime(root);
		c1.roll(Calendar.DAY_OF_MONTH, soNgay);
		System.out.println("Date before " + soNgay + " is : " + new SimpleDateFormat(sdf).format(c1.getTime()));
	}
	...
	public static String getDateTimeForPrint(String dateTime) {
		...
		String ret = "";
		try {
			ret = dateTime.substring(0, 2) + " gi\u1EDD " + dateTime.substring(3, 5) + " ph\u00FAt ng\u00E0y "
					+ dateTime.substring(6, 8) + " th\u00E1ng " + dateTime.substring(9, 11) + " n\u0103m "
	...
	public static Date str2ddsmmsyyyspHHmmss(String createDateStr) throws ParseException {
		if (Formater.isNull(createDateStr))
			return null;
		return sdfddsmmsyyyspHHmmss.parse(createDateStr);
	}
